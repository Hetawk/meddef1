...
class AttackLoader:
    ...
    def get_attack(self, attack_names):
        ...
        for attack_name in attack_names:
            try:
                ...
                if key in ['pgd', 'bim']:
                    ...
                    attacks.append(self.supported_attacks[key](self.model, epsilon, alpha, steps))
                
                elif key == 'cw':
                    c = getattr(self.config, 'attack_c', 1.0)
                    iterations = getattr(self.config, 'iterations', 40)  # Use the provided iterations value
                    ...
                    attacks.append(self.supported_attacks[key](self.model, epsilon, c, iterations, lr, binary_search_steps, confidence))
                
                elif key == 'zoo':
                    iterations = getattr(self.config, 'iterations', 40)  # Use the provided iterations value
                    h = getattr(self.config, 'attack_h', 0.001)
                    binary_search_steps = getattr(self.config, 'attack_binary_steps', 5)
                    attacks.append(self.supported_attacks[key](self.model, epsilon, iterations, h, binary_search_steps))
                    
                elif key == 'boundary':
                    steps = getattr(self.config, 'attack_steps', 40)  # Use 40 as default as specified
                    ...
                    attacks.append(self.supported_attacks[key](self.model, epsilon, steps, spherical_step, source_step, step_adaptation, max_directions))
                    
                elif key == 'elasticnet':
                    alpha = getattr(self.config, 'attack_alpha', 0.01)
                    iterations = getattr(self.config, 'iterations', 40)  # Use the provided iterations value
                    beta = getattr(self.config, 'attack_beta', 1.0)
                    attacks.append(self.supported_attacks[key](self.model, epsilon, alpha, iterations, beta))
                    
                elif key == 'onepixel':
                    pixel_count = getattr(self.config, 'attack_pixel_count', 1)
                    max_iter = getattr(self.config, 'iterations', 40)  # Use the provided iterations value
                    popsize = getattr(self.config, 'attack_popsize', 10)
                    attacks.append(self.supported_attacks[key](self.model, epsilon, pixel_count, max_iter, popsize))
                    
                else:
                    # For FGSM and JSMA
                    attacks.append(self.supported_attacks[key](self.model, epsilon))

            except Exception as e:
                logging.exception(f"Error initializing attack {attack_name}:")
                
        if not attacks:
            logging.error(f"Could not initialize any of the requested attacks: {attack_names}")
            return None
        ...
        if len(attacks) == 1:
            ...
            logging.info(f"Initialized composite attack with: {', '.join(attack_names)}")
