...
from model.attention.base_robust_method import BaseRobustMethod  # Assuming this exists in your codebase
...
class DenseBlock(nn.Module):
    def __init__(self, num_convs: int, in_channels: int, growth_rate: int):
    ...
    def _make_layer(self, in_channels: int, growth_rate: int) -> nn.Module:
...
class DenseNetModel(nn.Module):
    def __init__(self, growth_rate: int, block_config: List[int], num_classes: int,
        ...
        for i, num_convs in enumerate(block_config):
            self.blocks.append(DenseBlock(num_convs, num_channels, growth_rate))
    ...
    def load_pretrained_weights(self):
        pretrained_dict = load_state_dict_from_url(model_urls[f'densenet{self.growth_rate}'], progress=True)
