...
from model.meddef.cbamsa import CBAMBasicBlock, CBAMBottleneckBlock

class ResNetCBAMSA(nn.Module):
...
@check_num_classes
def get_rcbamsa(depth: int, input_channels: int = 3, num_classes: int = None,
                      robust_method: Optional[BaseRobustMethod] = None) -> ResNetCBAMSA:
    ...
    if depth not in depth_to_block_layers:
        raise ValueError(f"Unsupported rcbamsa depth: {depth}")

    block, layers = depth_to_block_layers[depth]
    return ResNetCBAMSA(block, layers, num_classes=num_classes, input_channels=input_channels,
